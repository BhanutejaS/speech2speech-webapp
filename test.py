...
PAUSE_THRESHOLD = 1.0  # seconds of silence to consider speech ended
RMS_HISTORY = 30
...
if not os.path.exists(csv_file_path):
    with open(csv_file_path, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["SegmentIndex", "STT_Text", "SpeechEndTime", "GPTStartTime", "LatencySeconds"]
        )
            ...
            if hasattr(delta, "content") and delta.content:
                if first_chunk:
                    ...
                    with open(csv_file_path, mode="a", newline="", encoding="utf-8") as f:
                        writer = csv.writer(f)
                        writer.writerow(
                            [segment_index, segment_text, speech_end_time, gpt_start_time, latency]
                        )
    ...
    try:
        with sd.InputStream(
            ...
            while True:
                ...
                # Detect speech activity (simple energy threshold)
                if np.max(np.abs(audio_data)) > 0.01:
                    last_speech_time = time.time()

                # Trigger after 1 sec pause
                if buffer.size > 0 and last_speech_time and (time.time() - last_speech_time > 1.0):
                    audio_float = buffer.copy()
                    buffer = np.zeros((0,), dtype=np.float32)  # reset buffer
                    last_speech_time = None  # reset pause tracker
