...
import random  # for choosing a random gap filler
...
# Gap filler phrases
GAP_FILLERS = ["Mhm.", "Uh-huh.", "Just a moment.", "Let me see.", "Right."]
...
# Gap Filler Synthesis
def synthesize_and_play_gap_filler():
    """
    Synthesizes a random gap filler phrase and pushes audio directly to the play queue.
    Runs in a non-blocking thread to cover LLM latency.
    """
    filler_text = random.choice(GAP_FILLERS)
    print(f"[Filler] Playing: {filler_text}")
    ...
    try:
        ...
        print(f"[ElevenLabs API Error - Filler] {e}")
        return
    ...
    for b in byte_iter:
        ...
        i = 0
        while i < x.size:
            # No play_start/play_end markers for filler
...
def tts_synth_worker():
    ...
    while True:
        try:
            ...
            if STOP_SIGNAL.is_set():
                # If we're interrupted and the queue is empty, loop again
                continue
            continue
...
def interrupt_response():
    ...
    TTS_PLAYING = False
    RESPONSE_GEN_ID += 1  # Invalidate any pending markers for the interrupted response
...
def gpt_stream_and_queue_tts(segment_text: str, pipeline_t0: float):
    ...
    # Run filler synthesis in a new, non-blocking thread to cover LLM network latency.
    if not TTS_PLAYING:
        threading.Thread(target=synthesize_and_play_gap_filler, daemon=True).start()
        # Small delay to let filler start synthesizing before the LLM call
        time.sleep(0.1)
            ...
            while True:
                ...
                if enough_audio and paused_long:
                    ...
                    if text:
                        ...
                        socketio.emit(
                            "transcript_update",
                            {"text": text, "t_stt": latency_stats["stt_latency_ms"]},
                        )
