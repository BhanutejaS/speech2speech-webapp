...
MIC_COOLDOWN_UNTIL = 0.0  # timestamp (seconds)
...
def synthesize_and_play_gap_filler():
    """
    Synthesizes a short 'thinking' filler phrase using ElevenLabs.
    Called asynchronously to mask LLM/TTS latency.
    """
    if STOP_SIGNAL.is_set():
        return  # do not play filler during interrupt or stop
    filler_text = random.choice(GAP_FILLERS)
    print(f"[Filler] Playing gap filler: {filler_text}")

    try:
        # Stream filler directly from ElevenLabs
        ...
        print(f"[ElevenLabs filler error] {e}")
        return
    ...
    for b in byte_iter:
        ...
        pcm = buf[:usable]
        del buf[:usable]
        x = np.frombuffer(pcm, dtype="<i2")  # int16 mono PCM
        i = 0
        while i < x.size:
...
def looks_useful_text(t: str) -> bool:
    """
    Filters out short, filler, or echo-like utterances
    to avoid Whisper capturing the assistant's own TTS output.
    """
    global last_user_text, conversation_context

    raw = t.strip()
    if len(raw) < 3:
        return False

    # Ignore common one-word fillers
    fillers = {"ok", "okay", "right", "yeah", "yep", "sure", "alright"}
    if raw.lower() in fillers:
        print(f"[FILTER: filler echo] '{raw}'")
        return False

    # Block speech similar to last assistant output (echo)
    if len(conversation_context) > 1:
        last_assistant = conversation_context[-1]["content"].strip().lower()
        sim = difflib.SequenceMatcher(None, raw.lower(), last_assistant).ratio()
        if sim > 0.8:
            print(f"[FILTER: echo similarity {sim:.2f}] '{raw}'")
            return False
...
def tts_play_worker():
    ...
    while True:
        ...
        if isinstance(item, dict) and item.get("type") == "play_end":
           ...
           # --- Add microphone cooldown after response completes ---
           global MIC_COOLDOWN_UNTIL
           # --- Adaptive post-playback mic cooldown ---
           extra_delay = 0.8 if TTS_PLAY_RMS > 0.05 else 0.4
           MIC_COOLDOWN_UNTIL = time.time() + 1.2 + extra_delay
           print(f"[POST-TTS] Mic cooldown for {1.2 + extra_delay:.1f}s after playback (adaptive to volume).")
...
def interrupt_response():
    global TTS_PLAYING, RESPONSE_GEN_ID, MIC_COOLDOWN_UNTIL
    STOP_SIGNAL.set()
    ...
    socketio.start_background_task(
        socketio.emit,
        'assistant_stream',
        {'token': "Oh, sorry — you go ahead."}
    )
    socketio.start_background_task(
        socketio.emit,
        'status',
        {'message': "Response interrupted. Listening..."}
    )

    # Add microphone cooldown (ignore audio for 800 ms)
    MIC_COOLDOWN_UNTIL = time.time() + 1.0 

    print("[INTERRUPT] Response stopped. Mic cooldown active for 800 ms.")
# ====================== LLM STREAMING ======================
def gpt_stream_and_queue_tts(segment_text: str, pipeline_t0: float):
    global conversation_context, RESPONSE_GEN_ID, rms_values
    if not TTS_PLAYING and not STOP_SIGNAL.is_set():
        threading.Thread(target=synthesize_and_play_gap_filler, daemon=True).start()
        time.sleep(0.1)
    with LLM_LOCK:
        STOP_SIGNAL.clear()
        ...
        with sd.InputStream(samplerate=cfg.sample_rate, channels=1, blocksize=cfg.block_size, callback=audio_callback):
            while True:
                ...
                try:
                    ...
                    # --- Adaptive echo-tail suppression (cross-device safe) ---
                    mic_rms = float(np.sqrt(np.mean(audio**2) + 1e-10))

                   # Skip input if still in cooldown
                    if time.time() < MIC_COOLDOWN_UNTIL:
                        continue

                   #  Auto-reset baseline if mic RMS stays extremely low
                    if mic_rms < 5e-5 and BACKGROUND_RMS < 1e-4:
                        BACKGROUND_RMS = 1e-3  # reset to safe minimum
                        continue

                    # Only suppress tiny trailing audio immediately after TTS playback
                    time_since_tts = time.time() - TTS_LAST_START_TIME
                    if (not TTS_PLAYING) and (time_since_tts < 2.5) and mic_rms < BACKGROUND_RMS * 0.8:
                        continue
